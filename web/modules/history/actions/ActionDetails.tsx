...
// local stubs to avoid pulling in the full Swap module
const isOrderCancelled = (status: string) => status === 'CANCELLED';
const isOrderExpired = (status: string) => status === 'EXPIRED';
const isOrderFilled = (status: string) => status === 'FULFILLED';
const isOrderLoading = (status: string) =>
  status === 'PENDING' || status === 'PRESIGNATURE_PENDING';
const swapTypesThatRequiresInvertedQuote: string[] = [];
