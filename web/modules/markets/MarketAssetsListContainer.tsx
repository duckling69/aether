...
import { AssetCategoryMultiSelect } from 'components/AssetCategoryMultiselect';
...
import { TitleWithFiltersAndSearchBar } from 'components/TitleWithFiltersAndSearchBar';
import { TitleWithSearchBar } from 'components/TitleWithSearchBar';
import { useAppDataContext } from 'hooks/app-data-provider/useAppDataProvider';
import { useCoingeckoCategories } from 'hooks/useCoinGeckoCategories';
import { CustomMarket } from 'ui-config/marketsConfig';
import MarketAssetsList from 'modules/markets/MarketAssetsList';
import { useRootStore } from 'store/root';
import { GHO_MINTING_MARKETS, GHO_SYMBOL } from 'utils/ghoUtilities';
...
export const MarketAssetsListContainer = () => {
  ...
  const [showLowLiquidityToggle, setShowLowLiquidityToggle] = useState(currentMarket === CustomMarket.proto_robinhood_rwa);
  ...
  return (
    ...
    >
      ...
      {!loading && filteredData.length === 0 && !displayGhoBanner && account && (
        <NoSearchResults
      ...
      {!loading && filteredData.length === 0 && !displayGhoBanner && !account && (
        <Box sx={{ mt: 4 }}>
          <ConnectWalletPaper
            ...
            description={
              <Trans>
                Connect your wallet to load the asset list and see your available market actions.
              </Trans>
            }
          />
        </Box>
