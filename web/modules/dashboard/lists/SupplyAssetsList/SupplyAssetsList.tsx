...
import { AssetCategory, isAssetInCategoryDynamic } from 'modules/markets/utils/assetCategories';
import { API_ETH_MOCK_ADDRESS, USD_DECIMALS, valueToBigNumber } from 'protocol/aave-compat';
  ...
  const tokensToSupply = reserves
    ...
    .map((reserve: ComputedReserveData) => {
      ...
      const hasDifferentCollateral = user?.userReservesData.find(
        (userRes: any) => userRes.usageAsCollateralEnabledOnUser && userRes.reserve.id !== reserve.id
      );

      // Check if asset has non-zero liquidation threshold (base or in user's e-mode)
      const userEMode = reserve.eModes?.find((e: any) => e.id === user?.userEmodeCategoryId);
