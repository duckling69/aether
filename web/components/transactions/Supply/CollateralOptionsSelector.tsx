...
import { EmodeAssetTable } from 'components/transactions/Emode/EmodeAssetTable';
...
function getBlockReason(
  ...
  if (targetEmodeId !== 0 && targetEmode) {
    ...
    const incompatibleBorrow = user.userReservesData.find(
      (ur: { scaledVariableDebt: string; reserve: { underlyingAsset: string } }) =>
  ...
  for (const userReserve of user.userReservesData) {
    ...
    if (targetEmodeId === 0) {
      ...
      const reserveTargetEmode = reserve.eModes.find((e: { id: number }) => e.id === targetEmodeId);
      if (
        reserveTargetEmode &&
  ...
  poolReserve.eModes.forEach(
    (reserveEmode: {
      ...
      if (reserveEmode.collateralEnabled) {
        ltv = reserveEmode.ltvzeroEnabled ? 0 : Number(reserveEmode.eMode?.ltv ?? 0) / 10000;
      } else {
