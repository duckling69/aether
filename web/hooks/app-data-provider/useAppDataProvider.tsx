...
export const AppDataProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const sdkMarket = data?.find(
    (item: { address: string }) => item.address.toLowerCase() === marketAddress
  );

  const totalBorrows = sdkMarket?.borrowReserves.reduce(
    (acc: number, reserve: { borrowInfo?: { total?: { usd?: number } } }) => {
  ...
  const supplyReserves = (sdkMarket?.supplyReserves ?? []).map(
    (reserve: { underlyingToken: { address: string } }) => ({
      ...reserve,
  ...
  const borrowReserves = (sdkMarket?.borrowReserves ?? []).map(
    (reserve: { underlyingToken: { address: string } }) => ({
      ...reserve,
      underlyingToken: patchUnderlyingToken(reserve),
      id: `${sdkMarket?.address}-${reserve.underlyingToken.address}`,
