...
import { FormatUserSummaryAndIncentivesResponse, valueToBigNumber } from 'protocol/aave-compat';
...
export function assetCanBeBorrowedByUser(
  ...
  if (user?.isInEmode) {
    const reserveEmode = eModes.find((emode: any) => emode.id === user.userEmodeCategoryId);
