...
import { normalizeBN, RAY, rayDiv, rayMul } from 'protocol/aave-compat';
...
const bisectDate = bisector((d: any) => d.utilization).center;
  ...
  for (let i = 0; i <= resolution; i++) {
    ...
    // When hovering above optimal utilization rate, hypothetical predictions
      ...
      const excess = rayDiv(
        ...
        Number(RAY) - Number(optimalUsageRatio)
      );
      const theoreticalVariableAPY = normalizeBN(
...
export const InterestRateModelGraph = withTooltip<AreaProps, TooltipData>(
  ({
    ...
    // Tooltip Styles
    const accentColorDark = theme.palette.mode === 'light' ? 'rgba(0, 0, 0, 0.12)' : 'rgba(255, 255, 255, 0.3)';
    const tooltipStyles = {
      ...defaultStyles,
      padding: '8px 12px',
      boxShadow: '0px 0px 2px rgba(0, 0, 0, 0.2), 0px 2px 10px rgba(0, 0, 0, 0.1)',
      borderRadius: '4px',
      color: theme.palette.mode === 'light' ? '#555555' : '#A0A0A0',
      fontSize: '12px',
      lineHeight: '16px',
    ...
    const yValueScale = useMemo(() => {
      const maxY = max(data, (d: any) => getVariableBorrowRate(d)) as number;
      return scaleLinear({
        range: [innerHeight, 0],
        domain: [0, (maxY || 0) * 1.1],
        nice: true,
