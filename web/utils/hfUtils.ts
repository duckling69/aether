...
export function calculateHFAfterSwap({
  ...
  const fromEmode = fromAssetData.eModes.find((elem: any) => elem.id === user.userEmodeCategoryId);
  ...
  const toEmode = toAssetData.eModes.find((elem: any) => elem.id === user.userEmodeCategoryId);
...
export const calculateHFAfterWithdraw = ({
  ...
  const userEMode = poolReserve.eModes.find((elem: any) => elem.id === user.userEmodeCategoryId);
...
export const calculateHFAfterSupply = (
  ...
  const userEmode = user
    ? poolReserve.eModes.find((e: any) => e.id === user.userEmodeCategoryId)
    : undefined;
