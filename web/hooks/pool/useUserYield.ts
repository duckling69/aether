...
import { FormatUserSummaryAndIncentivesResponse, ProtocolAction } from 'protocol/aave-compat';
        ...
        if (reserve) {
          if (value.underlyingBalanceUSD !== '0') {
            ...
            if (reserve.aIncentivesData) {
              reserve.aIncentivesData.forEach((incentive: any) => {
                acc.positiveProportion = acc.positiveProportion.plus(
          ...
          if (value.variableBorrowsUSD !== '0') {
            ...
            if (reserve.vIncentivesData) {
              reserve.vIncentivesData.forEach((incentive: any) => {
                acc.positiveProportion = acc.positiveProportion.plus(
...
export const useUserYields = (
  ...
  return poolsFormattedReservesQuery.map((elem: any, index: number) => {
    const selector = (
      formattedPoolReserves: FormattedReservesAndIncentives[],
      user: FormatUserSummaryAndIncentivesResponse
    ) => {
