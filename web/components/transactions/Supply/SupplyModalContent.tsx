...
import { useTokenInForTokenOut, useTokenOutForTokenIn } from 'protocol/hooks';
import { useRootStore } from 'store/root';
import { GENERAL } from 'utils/events';
import {
  getMaxAmountAvailableToSupply,
  remainingCap,
} from 'utils/getMaxAmountAvailableToSupply';
import { calculateHFAfterSupply } from 'utils/hfUtils';
import { isFeatureEnabled } from 'utils/marketsAndNetworksConfig';
import { replaceUnderscoresWithSpaces, roundToTokenDecimals } from 'utils/utils';
...
export const SupplyModalContentWrapper = (
  ...
  const hasDifferentCollateral = user.userReservesData.find(
    (reserve: { usageAsCollateralEnabledOnUser: boolean; reserve: { id: string } }) =>
      reserve.usageAsCollateralEnabledOnUser && reserve.reserve.id !== poolReserve.id
  );
...
export const SupplyModalContent = React.memo(
  ({
    ...
    const hasEmodeOptions =
      !poolReserve.isIsolated &&
      !user.isInIsolationMode &&
      poolReserve.eModes.filter(
        (e: { id: number; collateralEnabled: boolean }) => e.id !== 0 && e.collateralEnabled
      ).length > 0;
    ...
    const healfthFactorAfterSupply = (() => {
      if (needsEmodeSwitch) {
        ...
        const selectedEmode = poolReserve.eModes.find(
          (e: { id: number }) => e.id === selectedEmodeId
        );
        const reserveLT =
    ...
    const effectiveCollateralType = (() => {
      ...
      const targetEmode = poolReserve.eModes.find((e: { id: number }) => e.id === selectedEmodeId);
