...
export default function TopBarNotify({ campaigns, routeCampaigns }: TopBarNotifyProps) {
  ...
  const currentCampaign = routeCampaigns?.[pathname] ?? getCurrentCampaign() ?? null;
      ...
      >
        ...
        >
          <Box sx={{ padding: md ? '20px 10px' : '', paddingRight: 0 }}>
            <Typography
              sx={{ display: 'flex', alignContent: 'center', alignItems: 'center', color: '#0A0A0A', fontWeight: 500 }}
              component="div"
            >
              ...
              {currentCampaign.learnMoreLink && md ? (
                typeof currentCampaign.learnMoreLink === 'string' ? (
                  <Link
                    sx={{ color: '#0A0A0A', textDecoration: 'underline', fontWeight: 600, paddingLeft: 2 }}
          ...
          <Button
            sx={{ color: '#0A0A0A', paddingRight: 0 }}
            onClick={handleClose}
            startIcon={<CloseIcon />}
          />
        </Toolbar>
